...
def display_dashboard(start_date, end_date, target):
    ...
    with tab1:
        ...
        with col1:
            ...
            final = df.loc[df['Quarter'] == end_date, terget].item()
    ...
    with tab2:
        ...
        all_targets = st.multiselect('Choose other loaction', options= filtered_df.columns[1:], default=terget)
...
if __name__=="__main__":
    ...
    st.title("Built an interactive Streamlit dashboard analyzing Canadian population trends (1991–2025)")
    st.markdown("Source Table can be found here: (https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=1710000901)")

    with st.expander('Check full Data set'):
        st.write(df)
    ...
    with st.form('form_key'):
        ...
        with col1:
            ...
            start_year = st.slider("Year", min_value=1991, max_value=2025, value=1991, step=1, key="start_y")

        with col2:
            st.write('Choose a ending Date')
            end_quarter = st.selectbox('Quarter',options=["Q1", "Q2", "Q3", "Q4"], index=2, key="end_q" )
            end_year = st.slider('Year',min_value=2025, max_value=2025, value=1991, step=1, key="end_y")

        with col3:
            st.write('Choose Location')
            terget = st.selectbox("Choose a location",options=df.columns[1:],index=0)
    ...
    if start_date not in df['Quarter'].tolist() or end_date not in df['Quarter'].tolist():
        ...
        st.error('Does not work. Start date must come before end date.')
    else:
        display_dashboard(start_date,end_date, terget) 